...
def get_wcs_evt(fname):
    ...
    for key, val in header.items():
        if val == 'RA---TAN': ra_col=key[5:]
        if val == 'DEC--TAN': dec_col=key[5:]
    # fill in the wcs info.
    wcs_evt2.wcs.crpix = np.array( [header['TCRPX'+ra_col], header['TCRPX'+dec_col]] )
    wcs_evt2.wcs.cdelt = np.array( [header['TCDLT'+ra_col], header['TCDLT'+dec_col]] )
    wcs_evt2.wcs.cunit = np.array( [header['TCUNI'+ra_col], header['TCUNI'+dec_col]] )
    wcs_evt2.wcs.crval = np.array( [header['TCRVL'+ra_col], header['TCRVL'+dec_col]] )
...
def get_bfaft_cnts(evt_data_raw, src_xs, src_ys, R_apers, Elo=5e2, Eup=7e3):
    ...
    evt_idxs_edg  = np.where( (evt_data['time']< t_q1) | (evt_data['time']>=t_q3) )[0]
    evt_idxs_cent = np.where( (evt_data['time']>=t_q1) & (evt_data['time']< t_q3) )[0]
    # Extract event x, y
    ...
    for src_idx in range(Nsrc):
        ...
        if Ncnt_tot<5*Ncnt_bkg_scl: 
            Ncnt_bfs.append(-99);  Ncnt_afts.append(-99)
            Ncnt_edgs.append(-99); Ncnt_cents.append(-99)
        else:
            # Calculate distance square
            ...
            Ncnt_afts.append( len( np.intersect1d(evt_in_aper_idxs, evt_idxs_aft)  ) )  
            Ncnt_edgs.append( len( np.intersect1d(evt_in_aper_idxs, evt_idxs_edg)  ) )
            Ncnt_cents.append(len( np.intersect1d(evt_in_aper_idxs, evt_idxs_cent) ) )  
...
def get_tran_cand(Ncnt_bfs, Ncnt_afts):
    ...
    xt_flags[use_idxs] = \
        ...
        ((Ncnt_bfs[use_idxs]>5*Ncnt_afts[use_idxs]) | (Ncnt_afts[use_idxs]>5*Ncnt_bfs[use_idxs]))

    return xt_flags

def transient_selection(evt_data_raw, src_xs, src_ys, R_apers):
    Ncnt_bfs, Ncnt_afts, Ncnt_edgs, Ncnt_cents = get_bfaft_cnts(evt_data_raw, src_xs, src_ys, R_apers, Elo=5e2, Eup=7e3) 
    # Select candidate 
    # By N1 and N2
...
#    src_cat = queryCSC(obs_id)
    ...
    src_ras, src_decs, src_thetas= np.array(ra), np.array(dec), np.array(theta)
...
#    src_r0s, src_r1s= np.array(src_cat['err_ellipse_r0']), np.array(src_cat['err_ellipse_r1'])
    ...
    if(time<=20.0):
        ...
        #shift
            shift=10.0*1000
            low_lim,high_lim=(w*1000*i)+t_start+(shift),(w*1000*(i+1))+t_start+(shift)
            evt_data = evt_data_raw[np.where((evt_data_raw['time']>=low_lim) & \
                                    (evt_data_raw['time']<high_lim))]
            if(len(evt_data)!=0):
                xt_idxs=chopp(evt_data, src_xs, src_ys, R_apers)
        ...
        #res
#            if(res!=0.0):
#                low_lim,high_lim=(w*1000*i)+t_start+(res*1000),(w*1000*(i+1))+t_start+(res*1000)
#                evt_data = evt_data_raw[np.where((evt_data_raw['time']>=low_lim) & \
#                                    (evt_data_raw['time']<high_lim))]
#                print(len(evt_data))
#                xt_idxs=chopp(evt_data, src_xs, src_ys, R_apers)
...
#                    candidates.append(xt_idxs[j])
    ...
    for i in range(0,len(myFinalList)):
        g.write(str(obs_id)+' '+str(src_ras[myFinalList[i]])+' '+str(src_decs[myFinalList[i]])+' '+str(src_thetas[myFinalList[i]])+' '+str(src_pos[myFinalList[i]])+' '+str(src_sig[myFinalList[i]])+"\n")
    g.close()
...
def off_axis(event_file,x1,y1):
    command='dmcoords '+event_file+' option=sky x='+str(x1)+' y='+str(y1)+' verbose=1'
...
def search_candidates(src_file,event_file):
    ...
    for i in range(0,len(X)):
        angle=off_axis(event_file,X[i],Y[i])
        THETA.append(angle)
        err_pos.append(np.sqrt(RA_err[i]**2+DEC_err[i]**2)*3600)
